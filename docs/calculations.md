...
Blend valid estimates in logarithmic bucket space. An unavailable projection with positive weight makes the blend unavailable; its sentinel must never be treated as a fee. Fees are made nonincreasing with increasing targets: a valid shorter-horizon estimate is also usable for a longer horizon. The correction considers every whole-number horizon from 1 through the requested target, regardless of the configured or requested target set. This ensures single-target requests, batches, and different configurations return the same fee for the same horizon and confidence. Quantiles through the largest configured target are cached at construction; larger per-call horizons are computed without mutating shared state.
